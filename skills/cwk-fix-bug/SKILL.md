...
# cwk-fix-bug — bug (or QA report) → triage → root cause → surgical fix → regression test → back to QA

A focused hotfix pipeline, distinct from `/cwk-build` (which builds features). Production / a QA
...
- **Minimal, surgical, reversible.** Smallest diff that fixes the root cause; respect the
  ...
  discipline in `cwk-build`). For risky fixes prefer a guard/feature-flag and note a rollback.
...
- **Traceability** — the **failing test case ID / acceptance criterion / user story** it came from, if
  QA has one (from a `/cwk-qa` plan). This defines "correct".
...
- **Data / migration** → the env's data or schema/Flyway state differs (a migration not run, bad rows).
  → fix = migration/data, possibly via `/cwk-migrate`.
...
Also append ONE row to `cwk-sessions/fixes/_journal.md` (create with this header if missing) —
...
Save the report to `cwk-sessions/fixes/<slug>-<date>.md` (offer a PDF/HTML export via the cwk-pdf skill if useful):
...
- **Close the loop:** tell the user to **redeploy, then re-verify on the environment** — re-run the
  failing case, ideally with `/cwk-qa-integration` against the running app (or ask QA to re-test that
  ...
  `review-skills.md` so a future `/cwk-build` or `/cwk-review` catches it. Remind the user you don't
  deploy.
...
Fits the loop: `/cwk-build` → `/cwk-qa` (cases) → deploy → QA finds a bug → **`/cwk-fix-bug`** →
`/cwk-qa-integration` (re-verify on the env) → done.
