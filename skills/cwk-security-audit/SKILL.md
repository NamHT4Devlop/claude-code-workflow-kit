...
# cwk-security-audit — whole-repo security sweep

A repo-wide security audit (broader than `/cwk-review`, which is per-file). It enumerates the
**attack surface**, checks it against a security checklist, and reports prioritized findings with
fixes. Read-only — it does NOT change code (hand fixes to `/cwk-fix-bug` or `/cwk-build`).
...
2. For each category, inspect the relevant code (**grep callers** to follow untrusted
   data from entry → sink). Prefer fanning out the **`cwk-security-reviewer`** sub-agent on the
   high-risk areas.
...
Save to `cwk-sessions/security/audit-<date>.md`, then render to HTML and open it.
Resolve this skill's `references/` dir first (call it `$SKILL_DIR`): `${CLAUDE_PLUGIN_ROOT}/skills/cwk-security-audit/references`
if `CLAUDE_PLUGIN_ROOT` is set, else the `references/` folder next to this SKILL.md, else `$HOME/.claude/skills/cwk-security-audit/references`.
```bash
...
- For each [CRITICAL]/[MAJOR], offer to fix via `/cwk-fix-bug`.
