...
With an index: `provenlens hotspots` ranks the modules most depended on (start the Deep-Dive there, not alphabetically), and `provenlens cycles` turns "Cross-Module Communication" from a diagram of intent into a list of what actually happens.
...
**Every rule gets a stable id — this is not optional.** `/cwk-build`, `/cwk-qa` and
`/cwk-review` cite rules by id (`BR-V2`, `CF-03`) to tie a test or a finding to the rule it
...
6. **Architecture Invariants — DO NOT BREAK** — a numbered checklist of hard rules new code MUST satisfy (layering, naming, transaction boundaries, where validation lives, idempotency for consumers…). Mark `[CRITICAL]`/`[MAJOR]`. **This list is used to review every generated change.**
   **Prefer invariants that can be checked.** "Nothing may bypass `X`" is verifiable when `X` is a named symbol — `provenlens callers X` and `provenlens path <caller> <forbidden target>` settle it in one command — whereas "keep the layers clean" can only ever be argued about. Where an invariant rests on an edge the index cannot resolve, say so in the rule itself.

---

## 17 — `17-async-events.md` — Event / Contract Catalog (only if the service uses messaging)
The language-agnostic surface `/cwk-system-map`, `/cwk-qa` and `/cwk-build` use for
...
- `_coverage-report.md` — which files were analyzed vs covered by global scan only. Where an index exists, also record `provenlens status` and anything `provenlens doctor` flags as blocking: low coverage caused by an uninstalled dependency and low coverage caused by a resolver limit look identical in the number and are nothing alike for whoever reads this KB next.
