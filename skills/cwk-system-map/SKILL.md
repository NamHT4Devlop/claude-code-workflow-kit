...
# cwk-system-map — cross-service / end-to-end map for a microservices workspace

Per-service `/cwk-scan` (KB) understands each service **internally**. This skill adds the
...
- `provenlens path <symbol in A> <symbol in B>` walks a chain that crosses the repo boundary — that is
...
Resolve this skill's `references/` dir first (call it `$SKILL_DIR`): `${CLAUDE_PLUGIN_ROOT}/skills/cwk-system-map/references`
if `CLAUDE_PLUGIN_ROOT` is set, else the `references/` folder next to this SKILL.md, else `$HOME/.claude/skills/cwk-system-map/references`.
```bash
