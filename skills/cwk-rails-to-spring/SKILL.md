...
# cwk-rails-to-spring — behavior-preserving cross-stack port (parity-verified, multi-agent)

Distinct from `/cwk-migrate` (which evolves a contract **in place**, same codebase). This is a
...
**Here:**
- `provenlens explore "<resolver|controller action>"` on the **source** side — the real behaviour to
...
- **Extraction (per endpoint, parallel lenses)** — different agents catch different rules:
  1. **Resolver/controller logic** (`cwk-codebase-analyzer`) — orchestration, params, branching.
  ...
  4. **Side-effects & flows** (`cwk-business-flow-tracer`) — DB writes to OTHER tables, SQS
...
- **Blast radius** (`cwk-impact-detector`) — who consumes this endpoint, so parity is enforced
...
Run/reuse `/cwk-scan` on the Rails repo → KB of business rules, flows, auth, and the Event/Contract
...
- **Report (dual-audience):** save to `cwk-sessions/port/<service>-<date>.md`, then render it with
  ...
  open risks, cutover checklist. Then `/cwk-scan` the new Java repo to seed its KB. The human owns
  deploy and cutover.
