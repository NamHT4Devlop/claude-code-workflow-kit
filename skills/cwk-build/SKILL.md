...
If no artifact was named, still Glob `cwk-sessions/{user-stories,plans,discovery,plan-reviews,qa}/`
...
- **Shared contracts & schema go through migrate discipline.** If the change touches a DB schema, a
  ...
  **expand → migrate → contract** pattern from `cwk-migrate` (additive first, backfill, only then
  remove/tighten; every step reversible; a deprecation window for consumers). Never do a
  rename-in-place or a destructive migration inside a build — hand it to `/cwk-migrate` if it's the
  bulk of the work.
...
`Task` tool (`agents/cwk-codebase-analyzer`, `cwk-impact-detector`, `cwk-business-flow-tracer`).
...
Review the plan against a fixed challenge list — delegate to `/cwk-plan-review`, or spawn ONE
...
- **Security** → `agents/cwk-security-reviewer` — input validation, injection, authn/authz, data exposure, crypto/secrets.
- **Architecture & pattern conformance** → `agents/cwk-architecture-reviewer` — does it violate an "Architecture Invariant"? same pattern as the surrounding module? forbidden dependency direction? Quote the rule broken.
- **Performance** → `agents/cwk-performance-reviewer` — N+1 queries, missing indexes, memory leaks, blocking/sequential calls, missing pagination/caching.
- **Business consistency** → `agents/cwk-business-consistency-reviewer` — rules intact, no logic silently removed, valid state transitions, API contract preserved, **every AC implemented**.
...
- **Operability** — does the new path emit a structured log with the correlation id, and errors with
  ...
  the fields `cwk-observe` and the team's log schema use. *(For UI changes also check: no hardcoded
...
Then append ONE row to `cwk-sessions/builds/_journal.md` (create with this header if missing) so a
future session — and `/cwk-ask` — can find what was built and why:
```markdown
...
Reflect the change back into `knowledge-base/` by running the **`/cwk-rescan` procedure scoped to the
...
- `/cwk-qa <story>` — turn the ACs into an executable test plan, including regression for the flows
  named in `01-plan/plan.md` §2 (pass the session path).
- `/cwk-qa-integration <url>` — verify the feature on a running app.
- `/cwk-review` — an independent second-pass review of the whole change.
- `/cwk-pr` — draft the PR description from the branch.
- `/cwk-migrate` — if the Breaking-change gate deferred a contract/schema change.
