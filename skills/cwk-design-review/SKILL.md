...
# cwk-design-review — UI/UX + accessibility review
...
## Output (dual-audience; save `cwk-sessions/design/<app>-<date>.md`; render HTML; keep screenshots)
Resolve this skill's `references/` dir first (call it `$SKILL_DIR`): `${CLAUDE_PLUGIN_ROOT}/skills/cwk-design-review/references`
if `CLAUDE_PLUGIN_ROOT` is set, else the `references/` folder next to this SKILL.md, else `$HOME/.claude/skills/cwk-design-review/references`.
```bash
