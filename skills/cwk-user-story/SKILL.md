...
# cwk-user-story — investigate deeply → features + granular user stories
...
- `provenlens impact <touched symbol>` → **Dependencies** and **Assumptions** stop being guesses and
...
yourself:
- **`cwk-business-flow-tracer`** — which existing business flows the requirement interacts with,
  the NEW flow it introduces, applicable business rules, and state-machine impact.
- **`cwk-codebase-analyzer`** — the current implementation: real entities/fields, endpoints,
  services, reusable components, patterns and conventions the story must follow.
- **`cwk-impact-detector`** — blast radius: files/consumers affected, API/DB/event impact,
...
- No KB: fall back to Read/Grep/Glob over the source and **say the grounding is
  weaker** (likely misses non-obvious consumers); suggest `/cwk-scan`.
...
Save to `cwk-sessions/user-stories/<slug>-<date>.md`, opening with a **plain-language summary**
...
`${CLAUDE_PLUGIN_ROOT}/skills/cwk-user-story/references` if `CLAUDE_PLUGIN_ROOT` is set, else the
`references/` folder next to this SKILL.md, else `$HOME/.claude/skills/cwk-user-story/references`.
```bash
node "$SKILL_DIR/render-html.cjs" \
  "<repo>/cwk-sessions/user-stories/<slug>-<date>.md" \
  "<repo>/cwk-sessions/user-stories/<slug>-<date>.html" "<title>"
