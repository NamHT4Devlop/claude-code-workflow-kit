...
7. **Output** — write artifacts under `cwk-sessions/plans/`:
...
8. **Show in chat + render HTML.** Present the executive summary + backlog highlights in chat,
   ...
   Resolve this skill's `references/` dir first (call it `$SKILL_DIR`): `${CLAUDE_PLUGIN_ROOT}/skills/cwk-plan/references`
   if `CLAUDE_PLUGIN_ROOT` is set, else the `references/` folder next to this SKILL.md, else `$HOME/.claude/skills/cwk-plan/references`.
   ```bash
   node "$SKILL_DIR/render-html.cjs" \
     "<repo>/cwk-sessions/plans/user-stories-<epic>-<date>.md" \
     "<repo>/cwk-sessions/plans/user-stories-<epic>-<date>.html" "<epic title>"
...
granular ACs (or a Slack-thread source), use `/cwk-user-story` instead.
