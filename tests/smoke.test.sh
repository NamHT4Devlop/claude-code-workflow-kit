...
why=$(node -e '
  const { buildFromProvenlens, uncoveredLanguages } = require("./skills/namht-map/references/provenlens-graph.cjs");
...
held=$(node -e '
  const { indexedLanguages } = require("./skills/namht-map/references/provenlens-graph.cjs");
...
printf 'def handler():\n    return 1\n' > "$TMP/proj/worker.py"
langs=$(node -e '
  const { uncoveredLanguages } = require("./skills/namht-map/references/provenlens-graph.cjs");
